...
+ [Defining Resource Requests/Limits](#defining-resource-requestslimits)
...
and will be augment with environment, product, version, and deployment ID information.
...
> The `deployment.yaml`, files in `s3`, and `timer-*.json` configuration files will be processed
> using `envsubst` to provide environment specific values to be injected into the yaml.
> The environment variables from `deployment.conf` and the environment specific conf,
...
>
> `K8S_ENVIRONMENT`  
> The Kubernetes environment name, e.g. `qa`, `staging`, `production`, `staging-lab`, `lab`, or `uat`
>
> `K8S_DEPLOYMENT_ID`  
> A unique string ID generated by the Jenkins orchestration pipeline for this particular deployment.
>
> `K8S_LOAD_BALANCER`  
